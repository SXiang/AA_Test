...
public class FileUtil {
	...
	public static Locale locale = FileUtil.getSystemLocale();
	//**************  CMD operations ********************************
    ...
    public static String copyFile(String source, String dest, boolean confirm){	
    	...
    	result = exeComm(comm);
    	if(confirm&&!new File(dest).exists()&&!dest.toUpperCase().matches(appFile)) // it only works for character files
    	    writeFileContents(dest, getFileContents(source));
    	return result;    	
    }
...
//0817			PackageLoggingController.logPackageError(PackageLoggingController.PACKAGELOGLEVEL_ERRORS_ONLY, "Error in FileOps#readFile: " + e.getMessage());
		}
		return file;
	}
    //**************   Fils operations ***************************
...
//0817			PackageLoggingController.logPackageError(PackageLoggingController.PACKAGELOGLEVEL_ERRORS_ONLY, "Error in FileOps#getFileContents: " + e.getMessage());
			return "";
		}
	}
...
//0817				PackageLoggingController.logPackageError(PackageLoggingController.PACKAGELOGLEVEL_ERRORS_ONLY, "Error in FileOps#appendStringToFile: " + e.getMessage());
			}
		}
