...
SET STANDALONG=TRUE
IF "%SILENT_INSTALL%"=="" SET SILENT_INSTALL=FASE
...
SET COPY_DIR=D:\ACL\Analytics10_Binary\%Project%
MKDIR %COPY_DIR% 2>NUL
IF NOT EXIST %COPY_DIR% SET COPY_DIR=C:\ACL\Analytics10_Binary\%Project%
IF '"%DESROOT%"'=='""' (
	      ...
	      IF /I '%SILENT_INSTALL%'=='TRUE' (
		    set dir_suffix=Installer
 		    IF /I Not '%LOCALE%'=='En' (
			   set sub_Type=
 			   set sub_dir=%%f
 			   IF NOT EXIST %SRCROOT%\%%f\%dir_suffix% Call :getSubDir
 			) ELSE (
...
IF /I '%SupportNonUni%' == 'True' (
   ...
   ECHO.*    NonUnicode[%Locale%] - There is no such build %SupportNonUni%   ........
)
IF /I '%SupportUni%' == 'True' (
   ...
   ECHO.*    Unicode[%Locale%] - There is no such build %SupportUni%   .........
)
ECHO.*    Refresh ........................................R
ECHO.*       
...
:GETBUILD
   SET ACLscriptdir=%FILE_SERVER%\%Team_Name%\userContent\CI_Content\RFTBats
   SET ACLshortcutName="ACL Analytics 10"
   SET ACLdescription=%ACLExecutable%-%Version%[ACLQA_Automation]

   
   SET IHscriptdir=%FILE_SERVER%\%Team_Name%\userContent\\CI_Content\RFTBats
