@ECHO OFF
rem Iryna_Babak@ACL.com;Yousef_Aichour@ACL.com;
...
IF EXIST %HistoryDir%\%thisID%.robot set /p thisRobotXML=<%HistoryDir%\%thisID%.robot 2>NUL
IF EXIST %reportFile%.robot set /p oldRobotXML=<%reportFile%.robot 2>NUL

SET combinedXML=%oldRobotXML% %thisRobotXML%
rem IF '%oldRobotXML%'=='' SET combinedXML=%thisRobotXML%
rem IF '%thisRobotXML%'=='' SET combinedXML=%oldRobotXML%
Echo.********* combine output.xml ********
Echo. reportFile.robot = '%reportFile%.robot'
Echo. historyDir\thisID.robot= '%historyDir%\%thisID%.robot'
Echo.   oldRobotXML = '%oldRobotXML%'
Echo.   thisRobotXML = '%thisRobotXML%'
Echo.   combinedXML = '%combinedXML%'
Echo.*************************************
ECHO. %combinedXML%> %reportFile%.robot 2>NUL
...
:: Don't need these for script testing --- Steven
SET TABLE_TYPE=
...
echo. "%WORKSPACE%"\robotbats\CDOMessage.exe %subject%%d%%smtpServer%%d%%fromName%%d%%fromAddress%%d%%toAddress%%d%%body%%d%%attachFiles%%d%%ccAddress%%d%%bccAddress%%d%%importance%%d%%userName%%d%%password%%d%%ipPort%%d%%ssl%
...
Call "%WORKSPACE%"\robotbats\CDOMessage.exe %emailCmd%
GOTO END
