...
Rem SET Nameprefix=ACLAnalytics10
...
:INSTALLER
  ...
  SET ACLNonUni=%NamePrefix%%LOCALE%_%ACLNonUni%
  SET ACLUni=%NamePrefix%%LOCALE%_%ACLUni%
...
REM ECHO.*         [%LOCALE%]   En,De,Es,Pt,Fr,Ch,Ko,Jp,Pl
REM ECHO.*
...
IF /I '%SupportNonUni%' == 'True' (
   ...
   IF /I "%aclNonUniExist%"=="Ready" (
	   ...
	   ECHO.*    Install  %Version% - NonUni[with Ironhide]....GNI
   ) Else (
...
IF /I '%SupportUni%' == 'True' (
   ...
   IF /I "%aclUniExist%"=="Ready" (
	  ...
	  ECHO.*    Install  %Version% - Uni[with Ironhide].......GUI
   ) Else (
