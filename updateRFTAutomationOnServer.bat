...
SET SRCROOT=D:\ACL\TFSView\RFT_Automation\QA_Automation_2012_V2.0
::SET SRCROOT=D:\ACL\ClearCaseView\steven_xiang_QA_Automation\qa_automation\QA_Automation_2009_V1.0
SET BKFOLDER=D:\ACL\Automation\MyTestFramework_Backup\CurrentRFTAutomation
SET DESROOT=W:\QA_Automation_2012_V2.0\ACL_User
...
SET UPLOADTO1=Development
SET TOUSER=%UPLOADTO2% Iryna Yousef Sophia Karen Kwok Steven2 Danny
::SET TOPUBLIC=%UPLOADTO2% Iryna Yousef Sophia
SET FD1="D:\ACL\TFSView\RFT_Automation\QA_Automation_2012_V2.0\ACLQA_Automation\ACL_Desktop\DATA\ACLProject\BACKUP\*"
SET FD2=
SET FD3=
SET FD4=
SET FD5=
...
IF Exist %BKFOLDER% XCOPY %SRCROOT%.\%FROMProject% %BKFOLDER%\%FROMProject%.\ %XCSWITCH%
IF Exist %DESBACKUP% XCOPY %SRCROOT%.\%FROMProject% %DESBACKUP%.\%FROMProject%.\ %XCSWITCH%
...
If /I '%UPDATE_PROJECT%'=='TRUE' (
   XCOPY %FD1% %DESROOT%.\%UPLOADTO1%\%FROMProject%\ACL_Desktop\DATA\ACLProject\BACKUP\ %XCSWITCH_REPLACE%
   )
...
XCOPY %DESROOT%.\%UPLOADTO1%\%FROMProject%.\lib\acl\tool\updateACLDailyBuild.bat %DESROOT%.\ %XCSWITCH_%
XCOPY %DESROOT%.\%UPLOADTO1%\%FROMProject%.\lib\acl\tool\SilentInstallation_32bit_ACLv93.bat %DESROOT%.\ %XCSWITCH_%
XCOPY %DESROOT%.\%UPLOADTO1%\%FROMProject%.\lib\acl\tool\SilentInstallation_64bit_ACLv93.bat %DESROOT%.\ %XCSWITCH_%
...
If /I '%UPDATE_PROJECT%'=='TRUE' (
   FOR %%U IN (%TOAUTOTEST%) DO IF NOT '%%U'=='' XCOPY %DESROOT%.\%UPLOADTO1%\%FROMProject%\ACL_Desktop\DATA\ACLProject\BACKUP %DESROOT%.\%%U.\%FROMProject%.\ACL_Desktop\DATA\ACLProject\BACKUP\ %XCSWITCH%
   )
