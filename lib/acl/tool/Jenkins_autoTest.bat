...
IF Exist %tFolder% (
   ...
   ECHO "Run test on deployed build..." %ScriptDir%\Jenkins_doTest.bat
   START "Run test on deployed build..." /D%ScriptDir% /B /WAIT Jenkins_doTest.bat
) ELSE (
   ...
   START "Update ACL And Run Test..." /D%ScriptDir% /WAIT /B %ACL_UPDATE_SCRIPT% %TEST_BUILD% %_VersionPrefix% %DOMAIN_NAME% %USER_NAME% %PASSWORD%
