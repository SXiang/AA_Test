...
public class StevenConvertScripts {
	...
	private static String RFT_HELPER_FILE_DIR = "Y:/workspace/RFT_Localization/Scripts/SilverBolt_Localized/AXEM_Integration_merged/resources/AXAutomatedScripts/";
...
//		project = "Gateway";
		...
		RFT_HELPER_FILE_DIR = System.getProperty("user.dir")+"/resources/AX_"+project+"/Tasks/";
		String targetDir = "C:/temp/AX_"+project+"/Tasks/";
		String srcDir = System.getProperty("user.dir")+"/AX_"+project+"/Tasks/";
		...
		for (String curJavaFile : new File(srcDir).list(new FileFilter("java"))) {				
...
//			newContentsBuffer = sepRftObjDecAssignSection(curJavaFile, newContentsBuffer);
	...
	private static StringBuffer organizeDpVars(String curJavaFile, StringBuffer contentsBuffer) {
		...
		String replaceMethod = "getDpString(\"";
		...
		for (String curString : srcStringArr) {			
			// in current line
			while (curString.indexOf(searchPatternStart) != -1&&!curString.trim().contains("//")) {
				modified = true;
			...
			for (String curString : srcStringArr) {
				if (curString.matches(SECTION_INSERT_POINT)) {
					...
					for (Entry<String, String> entry : masterHm.entrySet()) {
						...
						tmpStr = "\t\tString " + entry.getKey() + " = " + entry.getValue().trim().replace("dpString","getDpString") + ";";
						tmpStr += "\t//\t<==" + searchPatternStart + NEW_DP_VAR_PREFIX + index + searchPatternEnd + "\n"; 
